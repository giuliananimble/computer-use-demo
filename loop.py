...
* You are using Windows with WSL (Windows Subsystem for Linux) and Ubuntu {platform.machine()} architecture.
...
def _make_api_tool_result(
    result: ToolResult | str | bytes, tool_use_id: str
) -> BetaToolResultBlockParam:
    ...
    if isinstance(result, str):
        is_error = True
        tool_result_content = [{"type": "text", "text": f"Error: {result}"}]
    elif isinstance(result, bytes):
        # Handle case where result is bytes (e.g., an image)
        tool_result_content.append(
            {
                "type": "image",
                "source": {
        ...
        # Handle case where result is ToolResult
        if result.error:
            is_error = True
                ...
                tool_result_content.append(
                    {
                        ...
                        "source": {
                            "type": "base64",
                            "media_type": "image/png",
                            "data": base64.b64encode(result.base64_image).decode('utf-8'),
                        },
                    }
                )
