...
def _render_message(
    ...
    print(type(message))
    is_tool_result = isinstance(message, bytes)  # Adjust this check if necessary
    ...
    with st.chat_message(sender):
        if is_tool_result:
            # Assuming message is bytes and contains an image
            try:
                # Decode the bytes and open the image
                image = Image.open(io.BytesIO(message))
                st.image(image, caption='Rendered Image', use_column_width=True)
            except Exception as e:
